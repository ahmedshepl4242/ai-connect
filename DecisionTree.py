        ...
        if not chance_node:
         ...
         for move in legal_moves:
            if self.valid_move(move,node.board): 
                ...
                self.generate_expect_tree(child_node, depth - 1, not is_maximizing,True,None)
        else:
            child_node = Node(node.board, move=None, score=0)
            node.children.append(child_node)
            self.generate_expect_tree(child_node, depth - 1,is_maximizing,False,[3,2,4,5,1])
            childnode = Node(node.board, move=None, score=0)
            node.children.append(childnode)
            self.generate_expect_tree(childnode, depth - 1,is_maximizing,False,[0,6])
